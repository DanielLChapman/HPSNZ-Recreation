	...
	componentDidMount() {
	    ...
	    for (var i = 0; i < numberOfEls; i++) {
			...
			anime({
				...
				width: ['1px', '10px'],
				height: ['1px', '10px'],
				left: [Math.floor(Math.random() * midScreenX)+midScreenX/(Math.random()*2+1) + 'px', Math.floor(Math.random() * Math.cos(angle) * radius + midScreenX) + 'px'],
				top: [Math.floor(Math.random() * midScreenY)+midScreenY/(Math.random()*2+1) + 'px', 
